...
set.seed(1); samp <- sample(population, 60)
```
...
**Question 1**
The distribution of this sample is normal. The typical size in my sample is around 1412-1450, assuming that "typical" corresponds to mean and median measures.
```{r sample_hist, eval=TRUE}
hist(samp, breaks=20)
summary(samp)
```
...
**Question 2**
Another student's distribution would not be the same as mine, since it was randomly sampled. I expect it to be similar since it is sampled from the same population.
...
**Question 4**
95% confidence means that when repeated samples are taken, the population mean falls within the 95% confidence interval for 95% of the samples.
...
**Question 5**
My confidence interval (1345.591, 1554.542) captures the true mean of 1499.69. 
...
**Question 6**
I expect 95% of those intervals to capture the true population mean, because the conditions for the confidence interval to be valid have been met.
...
-   Using the following function (which was downloaded with the data set), 
    ...
    We see 49 out of 50 (98%) of the confidence intervals include the true populaton mean. This proportion is not exactly equal to the confidence interval, because there are pnly 50 intervals. To get closer to the 95% confidence level, more intervals have to be tested.

```{r plot-ci1, eval=TRUE}
...
-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    ...
    I chose a confidence level of 90%, which has a critical value of 1.644854.
...
-   Calculate 50 confidence intervals at the confidence level you chose in the 
    ...
    For a 90% confidence level, 48 out of 50 confidenence intervals (96%) contain the population mean. This percentage is larger than the 90% level.
```{r ci1, eval=TRUE}
