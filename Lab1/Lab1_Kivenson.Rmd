...
**There are 9,569 males are in the sample.**
```{r table-gender-barplot, eval=TRUE}
table(cdc$gender)
table(cdc$gender)/nrow(cdc)
barplot(table(cdc$gender)/nrow(cdc))
```

```{r table-exerany-barplot, eval=TRUE}
barplot(table(cdc$exerany)/nrow(cdc))
```

**A proportion of 0.23285 of the sample reports being in excellent health.**
```{r table-genhlth-summary, eval=TRUE}
...
**The mosaic plot reveals that the proportion of men who have smoked at least 100 cigarettes is larger than the proportion of women who have smoked at least 100 cigarettes. In other words, men in this sample display stronger smoking habits than the women in this sample.**
...
**The box plot shows that individuals with lower BMIs tend to evaluate their general health as better than individuals with higher BMIs. The worse the health evaluation is, the more right-skewed the BMI box plot is. I am choosing to compare exercise to BMI, since exercise can help with weight managament. The figure seems to suggest that the median BMI of people who exercise is less than the BMI of those who do not.**

```{r boxplot-ageexer, eval=TRUE}
...
**The second histogram provides more granular bmi ranges.**
...
**The relationship between weight and desired weight is positive and linear.**
```{r scatter-weight, eval=TRUE}
plot(cdc$weight ~ cdc$wtdesire)
plot(cdc$weight ~ cdc$wtdesire, xlim=(c(50,350)))
```    
...
**wdiff is discrete numerical data. If wdiff is 0, a person wants to stay the same weight. If it is negative, the person wants to lose weight. If wdiff is positive, the person wants to gain weight**
...
**The center of the wdiff plot is between -10 and 0, and the plot is unimodal and left-skewed. This indicates that the majority of people in the sample either want to lose weight or stay at the same weight.** 
```{r bar-wdiff, eval=TRUE}
hist(wdiff, breaks = 100)
hist(wdiff, breaks = 100, xlim=(c(-200,100)))
```   
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
    
**In this sample, more men than women want to gain or keep a constant weight. Most women in this sample (72%) wanted to lose weight, while only 54.8% of men wanted to lose weight. The median of wdiff is also slightly lower for women, which also indicates that more women want to lost weight than men.**
```{r boxplot-wdiff-gender, eval=TRUE}
prop.table(table(cdc$gender, wdiff == 0),1)
prop.table(table(cdc$gender, wdiff > 0),1)
prop.table(table(cdc$gender, wdiff < 0),1)
...
**The mean and standard deviation of weights are 169.683 and 40.08097, respectively. The proportion of the weights that are within one standard deviation of the mean is 0.7076.**
```{r summary-weightsd, eval=TRUE}
mean_weight = mean(cdc$weight)
print(mean_weight)
sd_weight = sd(cdc$weight)
print(sd_weight)
weight_1sd <- subset(cdc, weight > (mean_weight - sd_weight) &
                       weight < (mean_weight + sd_weight))
nrow(weight_1sd) / nrow(cdc)
```
<div id="license">
