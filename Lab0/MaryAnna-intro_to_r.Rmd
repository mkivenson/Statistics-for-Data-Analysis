...
**The number of girls baptized over the years decreases from 1640 through 1650, but generally increases from 1660 onward.**
...
**The plot of proportion of boys over time shows considerable fluctuation over time, with no disernable trends.**

```{r plot-prop-boys-vs-year, eval=TRUE}
...
**Years 1940 - 2002 are included in this data set.**
```{r view-data-present, eval=TRUE}
present
```

**The dimensions of this data set are 63 rows by 3 columns.**
```{r dim-data-present, eval=TRUE}
dim(present)
```

**The column names are "year","boys", and "girls".**
```{r names-data-present, eval=TRUE}
names(present)
```

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?
**These counts are much higher than Arbuthnot's. While Arbuthnot's counts were in the thousands each year for boys and girls, the present data is in the millions.**
```{r calc-total-bapt-vars-present, eval=TRUE}
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    response.
**The data holds up Arbuthnot's observation that boys are being born in greater proportion than girls in the U.S. However, although the proportion of boys is still above 0.50, it is steadily decreasing.**
```{r plot-prop-boys-vs-year-present, eval=TRUE}
...
**We saw the most total number of births in the U.S. in 1961.**
```{r calc-max-bapt-vars-present, eval=TRUE}
