...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    **A streak length of 1 means that there is one hit until a miss occurs. A streak length of zero means that there are no hits - only misses.**
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    **Kobe's typical streak is 0 - meaning that he misses. His longest streak was four baskets.**
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
  ...
  **21 coins came up heads in my simulation.**
    
```{r table-sim_unfair_coin, eval=TRUE}
sim_unfair_coin
...
**The distribution of streak lengths is right skewed, with the center at zero. The typical streak length for this simulation is zero, and the longest streak of baskets is 4.** 
...
**I would expect the distribution to be similar, because according to the law of large numbers, the frequencies of events with the same likelihood of occurance will even out and converge to the expected ratio of outcomes. The two distributions should product similar ratios of outcomes after 133 experiments.**
...
**Kobe Bryant's distribution of streak lengths is similar to the distribution of streak lengths for the simulated shooter. Both distributions are centered at 0 and skew to the right, with a maximum of four shots as the longest streak. This shows that the hot hand model does not fit Kobe's shooting pattern. The simulation shows that with Kobe's accuracy rate, his streaks can be obtain by chance and not necessary through hot hands.**
